...
public class CardGame {
	public static void main(String[] args){
		...
		if (userInput.nextInt() == 1){//User vs computer
		userVsComputer(userDeck);
		} else {// Computer vs computer
		computerVsComputer(userDeck);
		}
		
	}

	
	private static void computerVsComputer(String[] userDeck) {
		Random r = new Random();
		int computerCardIndex, computer2CardIndex;
		int computerOneWins = 0, computerTwoWins = 0;
		for(int i = 0; i < 100; i++){
			computerCardIndex = r.nextInt(52);
			
			do{
			computer2CardIndex = r.nextInt(52);
			} while( computerCardIndex == computer2CardIndex);//Random gen until we get diffrent cards
			
			
			if (compareTo(userDeck, computer2CardIndex, computerCardIndex) == computerCardIndex){
				computerOneWins++;
			} else {
				computerTwoWins++;
			}	
		...
		if(computerOneWins > computerTwoWins){
			System.out.println("Computer One won with " + computerOneWins + " Wins");
		} else if (computerOneWins < computerTwoWins){
			System.out.println("Computer Two won with " + computerTwoWins + " Wins");
		} else {
			System.out.println("Computer One and Two tied with " + computerTwoWins + " Wins");
		}
	...
	private static void userVsComputer(String[] userDeck) {
		Random r = new Random();
		Scanner userInput = new Scanner(System.in); //Define the scanner
		int userCardIndex, computerCardIndex;
		
		System.out.println("Please enter your card index: (0/51)");
		userCardIndex = userInput.nextInt();
		
		do{
		computerCardIndex = r.nextInt(52);
		} while( computerCardIndex == userCardIndex);//Random gen until we get diffrent cards
		
		
		if (compareTo(userDeck, userCardIndex, computerCardIndex) == userCardIndex){
			System.out.println("You won with your " + userDeck[userCardIndex] + ", vs the computers " + userDeck[computerCardIndex]);
		} else {
			System.out.println("You lost with your " + userDeck[userCardIndex] + ", vs the computers " + userDeck[computerCardIndex]);
		}	
